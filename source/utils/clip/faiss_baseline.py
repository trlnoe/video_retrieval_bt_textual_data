...
def indexing_methods_faiss_v2():
    faiss_db = faiss.IndexFlatL2(512)
    db = []
    for folder_path in tqdm(args.clip_features_path_v2):
        # print(folder_path)
        for feat_npy in tqdm(os.listdir(folder_path)):
            # print(feat_npy)
            video_name = feat_npy.split('.')[0]
...
def main(args):
    ...
    query_list = query_reading(args.query_path)
    print("Num of Query: ",len(query_list))
    for _, query in tqdm(enumerate(query_list)):
        text = query[0]
        ...
        result_path_saving = args.result_csv + "query-" + str(_+61) +'.csv'
        submission_path_saving = args.submission_csv +  "query-" + str(_+61) +'.csv'
...
def get_parser():
    ...
    parser.add_argument(
        "--text", 
        default="An artist is painting a mask meticulously. Around him, there are many masks. The artist wears very simple honeycomb sandals. Then there is a close-up image of the masks. This type of mask is called the Mid-Autumn Festival paper mask.", 
        type=str, 
        help="Text Query",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--query_path", 
        default="/dataset/AIC2022/query/query-pack-3.csv", 
        type=str, 
        help="Query path",
    )
