...
def updateCurrentCode():
    ...
    if request.method == "POST":
        ...
        model = request.json['model_choosing']


    else:
        query = request.args.get('query')
        text = request.args.get('text')
        model = request.args.get('model_choosing')
    print(f"text: {text}, query: {query}, model: {model}")
    # init database 
    global db, faiss_db
    # preprocessing text 
    ...
    for instance in zip(I[0],D[0]):
        ...
        frames_folder = frames_folder.replace("AIC2022/3/", "AIC2022/2/")
        frames_folder = frames_folder.replace("AIC2022/4/", "AIC2022/2/")
        
        keyframe_id = sorted(os.listdir(frames_folder))[idx].split('.')[0]
        video_name = video_name + '.mp4'
        
        video_name = video_name.replace("AIC2022/3/", "AIC2022/2/")
        video_name = video_name.replace("AIC2022/4/", "AIC2022/2/")
